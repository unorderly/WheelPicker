    ...
    NSCollectionViewDelegate, NSCollectionViewDelegateFlowLayout, NSScrollViewDelegate where Value: Comparable {
        ...
        private static var itemIdentifier: NSUserInterfaceItemIdentifier {
            NSUserInterfaceItemIdentifier("wheelpicker.item")
        }
        ...
        override func layout() {
            ...
            if self.scrollIndex != self.selectedIndex,
               !((self.collectionView.enclosingScrollView as? ScrollView)?.isScrolling ?? false) {
                self.scrollToItem(at: self.selectedIndex, animated: false)
            }
        }
        ...
        func offsetForItem(at index: Int) -> CGFloat {
            var offset = self.cvLayout?.originalAttributesForItem(at: IndexPath(item: index, section: 0))?.frame
                .midY ?? 0
